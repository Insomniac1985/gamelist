...
function importHtml(settings) {
  ...
  function entitlementControl(game,index,error){const key=game.provider==='playstation'&&['Sony PlayStation 3','Sony PlayStation 4','Sony PlayStation 5'].includes(game.platform)?'psPlus':game.provider==='xbox'&&['Xbox 360','Xbox One'].includes(game.platform)?'gamesWithGold':'';if(!key)return '';const label=key==='psPlus'?'PlayStation Plus':'Games with Gold';return '<label class="import-entitlement"><input type="checkbox" data-entitlement="'+key+':'+(error?'errors':'ready')+':'+index+'" '+(game[key]?'checked':'')+'><span>'+label+'</span></label>'}
  function row(game,index,error){const actions=error?'<button class="game-action" data-manual="'+index+'" title="Add game">+</button><button class="game-action" data-remove-error="'+index+'" title="Remove">×</button>':'<button class="game-action" data-add-ready="'+index+'" title="Add game">+</button><button class="game-action" data-remove="'+index+'" title="Remove">×</button>';return '<li class="game"><img src="'+esc(icon(game))+'" alt=""><span><strong>'+esc(game.title)+'</strong><small>'+esc(game.platform)+(error?' · Metadata match failed':' · '+esc(game.metadata?.source||'Matched'))+'</small>'+entitlementControl(game,index,error)+'</span><span class="game-actions">'+actions+'</span></li>'}
