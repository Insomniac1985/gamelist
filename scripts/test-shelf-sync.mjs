...
assert.match(themeSource, /function renderThemeDialog[\s\S]*?Main color[\s\S]*?Accent color[\s\S]*?Gradient color[\s\S]*?Extra color[\s\S]*?Light or dark[\s\S]*?Enable background glow[\s\S]*?Gradient titles[\s\S]*?Accent font[\s\S]*?Glow 1[\s\S]*?Glow 2[\s\S]*?Gamelist icon[\s\S]*?Shelf icon[\s\S]*?Big logo[\s\S]*?Game app icon[\s\S]*?Custom Background[\s\S]*?Custom Owner Colors/, "Theme editor must expose custom colors, light/dark mode, glow controls, image uploads, icons, big logo, and owner colors");
