...
assert.match(sharedCss, /\.page-pull-switch[\s\S]*?touch-action:\s*none;[\s\S]*?\.page-pull-curtain[\s\S]*?transform:\s*translateY\(calc\(-102% \+ var\(--pull-distance, 0px\)\)\);[\s\S]*?body\.page-switching \.page-pull-curtain[\s\S]*?transform:\s*translateY\(0\);/, "Shared CSS must provide a draggable pull-down page transition");
