...
assert.match(themeSource, /function renderThemeDialog[\s\S]*?Main color[\s\S]*?Gradient color[\s\S]*?Accent color[\s\S]*?Light or dark[\s\S]*?Gradient titles[\s\S]*?Big logo[\s\S]*?Background[\s\S]*?Gamelist icon[\s\S]*?Shelf icon[\s\S]*?Game app icon[\s\S]*?Custom Owner Colors/, "Theme editor must expose custom colors, light/dark mode, image uploads, icons, big logo, and owner colors");
...
assert.match(sharedCss, /\.column-head h2\s*\{[\s\S]*?background: var\(--accent\);[\s\S]*?html\.theme-gradient \.column-head h2\s*\{[\s\S]*?linear-gradient\(135deg, var\(--title-gradient-start/, "Titles must be flat accent unless Gradient titles is enabled");
...
assert.match(sharedCss, /html\.theme-light \.settings-layout-card,[\s\S]*?\.theme-upload-card[\s\S]*?background: rgba\(255, 255, 255, 0\.74\);[\s\S]*?html\.theme-light input,[\s\S]*?\.settings-sync-card \.settings-visible-check[\s\S]*?background-color: rgba\(255, 255, 255, 0\.9\);/, "Light mode settings cards and controls must use stronger contrast");
