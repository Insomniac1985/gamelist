...
assert.match(themeSource, /function renderThemeDialog[\s\S]*?Main color[\s\S]*?Accent color[\s\S]*?Gradient titles[\s\S]*?Gradient color[\s\S]*?Light or dark[\s\S]*?Enable background glow[\s\S]*?Glow 1[\s\S]*?Glow 2[\s\S]*?Extra color[\s\S]*?Accent font[\s\S]*?Background[\s\S]*?Gamelist icon[\s\S]*?Shelf icon[\s\S]*?Big logo[\s\S]*?Game app icon[\s\S]*?Custom Owner Colors/, "Theme editor must expose custom colors, light/dark mode, glow controls, image uploads, icons, big logo, and owner colors");
...
assert.match(sharedCss, /html\.theme-light \.settings-layout-card,[\s\S]*?\.theme-upload-card[\s\S]*?background: rgba\(220, 226, 236, 0\.82\);[\s\S]*?html\.theme-light input,[\s\S]*?\.settings-sync-card \.settings-visible-check[\s\S]*?background-color: rgba\(206, 214, 226, 0\.92\);/, "Light mode settings cards and controls must use stronger contrast");
