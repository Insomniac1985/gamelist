...
assert.match(sharedCss, /\.page-pull-switch[\s\S]*?touch-action:\s*none;[\s\S]*?\.page-pull-switch::before[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(0\.42\) scaleY\(0\.28\);[\s\S]*?\.page-pull-switch span[\s\S]*?font-size:\s*13px;[\s\S]*?\.page-pull-switch:hover::before[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(1\) scaleY\(1\);[\s\S]*?\.page-pull-curtain[\s\S]*?transform:\s*translateY\(calc\(-102% \+ var\(--pull-distance, 0px\)\)\);[\s\S]*?\.page-pull-preview[\s\S]*?width:\s*100vw;[\s\S]*?filter:\s*blur\(var\(--pull-blur, 10px\)\);[\s\S]*?\.page-pull-frame[\s\S]*?height:\s*100vh;[\s\S]*?pointer-events:\s*none;[\s\S]*?body\.page-switching \.page-pull-curtain[\s\S]*?transform:\s*translateY\(0\);/, "Shared CSS must provide a draggable pull-down page transition with an expanding accent handle and full-viewport real blurred destination preview");
