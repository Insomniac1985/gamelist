...
assert.match(sharedCss, /\.page-pull-switch::before[\s\S]*?height:\s*35px;[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(0\.42\) scaleY\(0\.57\);[\s\S]*?\.page-pull-switch span[\s\S]*?font-size:\s*13px;[\s\S]*?\.page-pull-switch:hover::before[\s\S]*?background:\s*var\(--accent\);[\s\S]*?transform:\s*translateX\(-50%\) scaleX\(1\) scaleY\(1\);/, "Pull switch must use a pure accent 20px collapsed/35px expanded handle");
